...
;;;;INITIAL VELOCITY 0
;pxtharr = make_array(n_elements(pxtharr))
;pytharr = make_array(n_elements(pytharr))
;pztharr = make_array(n_elements(pztharr))
...
;Inject particles in one place with 0 velocity and time correlated fields

;injds[*] = 0
;inkds[*] = 0

;zinjarr = make_array(n_elements(zinjarr)) ;;;;;;;;;;;;;

;xinjarr = make_array(n_elements(xinjarr)) + xmin/2 ;;;;;;;;;;;;;;;;;;
;yinjarr = make_array(n_elements(yinjarr)) ;;;;;;;;;;;;;;;;
