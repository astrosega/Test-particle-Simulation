...
oplot, x(5000:5001)/Re, z(5000:5001)/Re, psym=8, col=4
...
oplot, y(5000:5001)/Re, z(5000:5001)/Re, psym=8, col=4
...
pro ree_record_flux, x, y, z, gam, indbad, nindbad, W0, box_str, $
  Flux_str=Flux_Str
...
FOR i = 0, nuse-1 do BEGIN
  j = iW(induse(i))
  ; CHECK LOCATION
  if (x(indbad(i)) LT box_str.xexitmin) then Flux_str.mx(j) = Flux_str.mx(j)+1
  if (x(indbad(i)) GT box_str.xexitmax) then Flux_str.px(j) = Flux_str.px(j)+1
  if (y(indbad(i)) LT box_str.yexitmin) then Flux_str.my(j) = Flux_str.my(j)+1
  if (y(indbad(i)) GT box_str.yexitmax) then Flux_str.py(j) = Flux_str.py(j)+1
  if (z(indbad(i)) LT box_str.zexitmin) then Flux_str.mz(j) = Flux_str.mz(j)+1
  if (z(indbad(i)) GT box_str.zexitmax) then Flux_str.pz(j) = Flux_str.pz(j)+1
ENDFOR
...
JetSize  = Dist_Str.JetSize*Re
YStart    = Dist_Str.YStart*Re

; SET UP INDUSE
IF keyword_set(turb) then BEGIN & $
  indturb = where( (abs(x(ind)) GE JetSize) AND (y(ind) GE YStart) AND $
    (x(ind) GT (Box_str.xturbmin+BuffSize)) AND $
    (x(ind) LT (Box_str.xturbmax-BuffSize)) AND $
    (x(ind) LT (Box_str.xturbmax-BuffSize)) AND $
    (y(ind) GT (Box_str.yturbmin+BuffSize)) AND $
    (y(ind) LT (Box_str.yturbmax-BuffSize)) AND $
...
;print,number
  ...
  if keyword_Set(ddd) then begin
    ...
    if keyword_Set(overplot) then begin
      ...
      ,dimensions=[1100,1100],margin=[0.2,0.33,0.2,0.2],/perspective,buffer=0,AXIS_STYLE=2,thick=3,rgb_table=2,VERT_COLORS=BYTSCL(findgen(n_elements(x[1:-1])*factor)), $
