...
pro inject_particles3D_setup, ninjarr, vth, DenZ0, DenInjZ, nEtimes, nBtimes, $
 ...
 seeds = [756,5675,55672,908,10890,13894,423434595,6900-966,61231239,86789,10080, 99123249, 7346484 ,7905477]
...
xinjarr = randomu(seeds[0], ninjarr, /double)*(Xmax-Xmin) + Xmin
yinjarr = randomu(seeds[1], ninjarr, /double)*(Ymax-Ymin) + Ymin

; Z IS COMPLICATED - MAKE SECH^2 ARRAY
dum     = randomu(seeds[2], ninjarr,/double)
ztemp   = 0.5D * alog((1D + dum)/(1D - dum))
sgn     = 2L*round(randomu(seeds[3], ninjarr,/double)) - 1L
...
indmix = where(randomu(seeds[4], ninjarr, /double) LT thresh, nindmix)
if nindmix GT 0 then $
  zinjarr(indmix) = randomu(seeds[5], nindmix, /double)*(Zmax-Zmin) + Zmin

; CREATE THERMAL ARRAYS
dum1     = randomn(seeds[6], ninjarr, /double)*vth
dum2     = randomn(seeds[7], ninjarr, /double)*vth
vth2arr  = sqrt(dum1*dum1+dum2*dum2)

; SET DEFAULT MOMENTUM
pxtharr  = randomn(seeds[8], ninjarr, /double)*vth
pytharr  = randomn(seeds[9], ninjarr, /double)*vth
pztharr  = randomn(seeds[10], ninjarr, /double)*vth

; SELECT THE FACE
dum     =  randomu(seeds[11], ninjarr, /double)
...
injds = long(randomu(seeds[12], ninjarr)*(nEtimes-1))
inkds = long(randomu(seeds[13], ninjarr)*(nBtimes-1))
...
pro load_particles3D, nload, vth, DenZ0, DenInjZ, nEtimes, nBtimes, $
  ...
  seeds = [1,5,2,8,10,13,45,666,69,89,100, 999, 7 ,777]
...
; FILL ARRAYS WITH DEFAULTS
x(nstart:nend) = randomu(seeds[0], nload, /double)*(Xmax-Xmin) + Xmin
y(nstart:nend) = randomu(seeds[1], nload, /double)*(Ymax-Ymin) + Ymin

; Z IS COMPLICATED - MAKE SECH^2 ARRAY
dum     = randomu(seeds[2], nload,/double)
ztemp   = 0.5D * alog((1D + dum)/(1D - dum))
sgn     = 2L*round(randomu(seeds[3], nload,/double)) - 1L
...
ZdistEff = 2D*tanh(Zmax/DenZ0)*DenZ0 ; INTEGRATION OF SECH^2
thresh = DenExtra*(Zmax-Zmin)/(ZdistEff*(1D - DenExtra) +  DenExtra*(Zmax-Zmin))
indmix = where(randomu(seeds[4], nload, /double) LT thresh, nindmix)
if nindmix GT 0 then $
  z(nstart+indmix) = randomu(seeds[5], nindmix, /double)*(Zmax-Zmin) + Zmin

px(nstart:nend) = randomn(seeds[6], nload, /double)*vth
py(nstart:nend)  = randomn(seeds[7], nload, /double)*vth
pz(nstart:nend)  = randomn(seeds[8], nload, /double)*vth

c = 3d8
c2 = c*c
gam(nstart:nend) = sqrt(px(nstart:nend)*px(nstart:nend) + $
  py(nstart:nend)*py(nstart:nend) + pz(nstart:nend)*pz(nstart:nend) +c2)/c
dsE(nstart:nend) = long(randomu(seeds[9], nload)*(nEtimes-1))
dsB(nstart:nend) = long(randomu(seeds[10], nload)*(nBtimes-1))
