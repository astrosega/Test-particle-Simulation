...
pro ree_2dplus_ion_sim, dt, it, jt, kt, npart, imax, nnew, nh, box_str, $
  ...
  import_index = import_index, trajectories=trajectories, highp = highp, maskhot = maskhot, masklowp = masklowp
  ...
  if keyword_set(import_index) then begin
    restore, 'import_indexL.sav'
    ;trajectories = make_array(imax+1, 12, n_elements(import_index))
    trajectoriesL = make_array(3, n_elements(import_index))
    track = 0
  endif
  ...
  if subb[0] ne -1 then begin
    trajectoriesL[0, import_index[subb]]  = px[import_index[subb]]
    trajectoriesL[1, import_index[subb]]  = py[import_index[subb]]
    trajectoriesL[2, import_index[subb]]  = pz[import_index[subb]]
  endif
  ...
  if keyword_set(import_index) and track then begin
