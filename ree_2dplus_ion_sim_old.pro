...
pro ree_2dplus_ion_sim_old, dt, it, jt, kt, npart, imax, nnew, nh, box_str, $
...
FOR i = 0L, imax-1 do BEGIN & $
  ...
  indbad = where( (x LT xexitmin) OR (x GT xexitmax) OR $
                  (y LT yexitmin) OR (y GT yexitmax) OR $
                  (z LT zexitmin) OR (z GT zexitmax), nindbad)
  if (nindbad GT 0) and keyword_set(record_flux) then $
    ree_record_flux, x, y, z, gam, indbad, nindbad, W0, box_str, $
      Flux_str=Flux_Str
  if nindbad GT 0 then gam(indbad) = 0D
